  ...
  return (
    <nav className="w-full fixed z-40">
      <div className={`px-4 md:px-16 py-6 flex flex-row items-center transition duration-500 ${showBackground ? 'bg-zinc-900 bg-opacity-90' : ''}`}>
        <Image src="/images/logo.png" className="h-4 lg:h-7" alt="Logo" width={100} height={100} priority />
        ...
        <div className="flex flex-row ml-auto gap-7 items-center">
          ...
          <div onClick={toggleAccountMenu} className="flex flex-row items-center gap-2 cursor-pointer relative">
            <div className="w-6 h-6 lg:w-10 lg:h-10 rounded-md overflow-hidden">
              <Image src="/images/default-blue.png" alt="" width={100} height={100} priority />
            </div>
            <BsChevronDown className={`w-4 text-white fill-white transition ${showAccountMenu ? 'rotate-180' : 'rotate-0'}`} />
            <AcountMenu visible={showAccountMenu} />
          </div>
        </div>
      </div>
    </nav>
